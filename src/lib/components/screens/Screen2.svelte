<script>
    import ChecklistCard from "$lib/components/ChecklistCard.svelte";
    import AlexHormozi from "$lib/assets/AlexHormozi.webp";
    import NextBtn from "$lib/components/NextBtn.svelte";
</script>

<div class="flex flex-col justify-center items-center w-full fill-height absolute">
    <div class="px-8 md:px-0 lg:mt-16">
        <h1 class="text-3xl md:text-5xl font-bold">
            Choose your checklists/criteria 📋
        </h1>
    
        <p class="mt-1 text-lg md:text-xl text-gray-500">
            Objectively evaluate your business idea based on winning checklists
        </p>
    </div>

    <div class="md:mt-20 w-5/6mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <ChecklistCard name="Paul Graham" img={AlexHormozi}/>
        <ChecklistCard name="Alex Hormozi" img={AlexHormozi}/>
        <ChecklistCard name="Paul Graham" img={AlexHormozi}/>
        <ChecklistCard name="Paul Graham" img={AlexHormozi}/>
    </div>

    <button class="mt-6 ml-6 w-5/6 text-start font-medium underline text-lg cursor-pointer">Create a custom checklist</button>

    <div class="mt-6 mr-40 self-end flex items-center gap-8">
        <button class="w-fit h-fit text-black text-xl font-medium hover:scale-95 transition-all cursor-pointer">Back</button>
        <NextBtn text="Evaluate"/>
    </div>

    
    
</div>

<style>
    @media (max-width: 640px) {
        .fill-height{
            flex: 1;
            display: flex;
            justify-content: center;
        }
    }
    @media (min-width: 640px) {
        .fill-height{
            height: 100%;
        }
    }
</style>